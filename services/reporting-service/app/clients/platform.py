"""Read-only clients for every platform service reports draw from
("DATA SOURCES").
...
credential. A report therefore cannot contain data the requesting user
could not have fetched themselves, which keeps RBAC enforced by the
...
def unwrap(payload: Any, result_path: str | None) -> list[dict[str, Any]]:
    ...
    a report should be able to table it.
    """
    current: Any = payload
        ...
        *,
        ...
        if len(rows) > self._max_rows:
            raise DependencyError(
                ...
                "or raise AIIOS_REPORTING_SERVICE_MAX_ROWS_PER_REPORT."
            )
        return rows

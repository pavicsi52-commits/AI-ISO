...
rather than being pushed into each source service's query string. That
is a deliberate trade with a real reason: the twelve data sources
docs/047 names do not share a filter grammar, so a single filter
expression could not be translated to all of them faithfully. Applying
one consistent, testable grammar here means "status equals firing"
...
where they exist -- that is what :class:`~app.reports.designer.schema
.DataQuery.params` is for -- so this layer refines a result set rather
than replacing server-side filtering with a full table scan.
"""
...
def apply_filters(
    ...
    Clauses combine with AND. That is the only combination docs/047's

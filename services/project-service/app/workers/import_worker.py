"""Background worker: bulk project import processing.

Per docs/034 "PERFORMANCE": "Queue Integration". Registered on this
...
separate deployable process, the same queue-backed-in-process pattern
``services/user-management-service``'s own ``import_worker`` established.
"""
...
IMPORT_QUEUE_NAME = "project_import_queue"

ImportServiceFactory = Callable[[], AbstractAsyncContextManager[ProjectImportService]]
...
def build_import_worker(service_factory: ImportServiceFactory) -> MessageHandler:
    ...
    *service_factory* is an async context manager factory: entering it
    ...
    :class:`~app.services.import_service.ProjectImportService` built on
    it -- see ``services/user-management-service``'s identical
    ...
    async def handle_import_job(message: QueueMessage) -> None:
        ...
        try:
            ...
            logger.exception(
                "Project import job failed.", extra={"extra_fields": {"job_id": str(job_id)}}
            )
            raise

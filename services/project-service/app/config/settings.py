...
class ProjectServiceSettings(BaseSettings):
    ...
    model_config = SettingsConfigDict(
        env_prefix="AIIOS_PROJECT_SERVICE_", env_file=".env", extra="ignore"
    )

    host: str = Field(default="0.0.0.0")
    port: int = Field(default=8005, ge=1, le=65_535)
    cors_allowed_origins: list[str] = Field(default_factory=list)
    import_export_bucket: str = Field(default="project-import-export")
    jwt_public_key_path: str = Field(default="keys/jwt_public_key.pem")
    statistics_cache_ttl_seconds: int = Field(default=300, ge=1)
...
class Settings:
    ...
    email: EmailSettings
    minio: MinioSettings
    service: ProjectServiceSettings


def build_settings(*, service: ProjectServiceSettings | None = None) -> Settings:
    ...
    return Settings(
        ...
        email=shared.email,
        minio=shared.minio,
        service=service if service is not None else ProjectServiceSettings(),
    )
...
__all__ = ["ProjectServiceSettings", "Settings", "build_settings", "get_settings"]

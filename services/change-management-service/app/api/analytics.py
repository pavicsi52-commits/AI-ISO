...
async def download_report(organization_id: UUID, report_id: UUID, reports: ReportSvc) -> Any:
    ...
    FastAPI's default JSON handling to serialise -- see Prompt 052's own
    notes on why pinning the whole route to ``PlainTextResponse`` 500s
    the one format this endpoint's own default is supposed to support.

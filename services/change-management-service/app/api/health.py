...
**PostgreSQL gates readiness.** Every operation this service performs
reads or writes the change catalogue, and a replica taking traffic
without one would answer "no pending changes" -- indistinguishable from
a genuinely quiet queue and far more dangerous than an error.

**Redis is reported but does not gate.** It backs the scheduler's
leader election; losing it makes the conflict, approval-expiry, and
maintenance sweeps late, not wrong.
"""
...
async def health(
    ...
    """Return the overall health of the change management service."""
    data = HealthStatus(
        status="healthy",

"""Shared fixtures for the configuration management service's test suite.
...
No Neo4j fixture exists here -- docs/039 names no graph concept for
this service, unlike ``services/asset-management-service``'s own
dependency graph.

**Git provider calls**: only Gitea is genuinely live-tested (a locally
run Docker container, matching AWS/moto's role in
...
documented response shapes, never a live account.

This service's own database (``aiios_configuration_management``) is
physically separate from every other AI-IOS service's. Redis test db
12 -- distinct from every other AI-IOS service's own test db (3
authentication, 4 user-management, 5 rbac, 6 organization, 7 project,
8 secrets-management, 9 inventory, 10 discovery, 11 asset-management).
"""
...
os.environ.setdefault("AIIOS_DATABASE_NAME", "aiios_configuration_management")
...
os.environ.setdefault("AIIOS_REDIS_DB", "12")
...
_TEST_KEY_DIR = Path(tempfile.gettempdir()) / "aiios_configuration_management_test_keys"
...
os.environ.setdefault(
    "AIIOS_CONFIGURATION_MANAGEMENT_SERVICE_JWT_PUBLIC_KEY_PATH", str(_TEST_PUBLIC_KEY_PATH)
)
os.environ.setdefault(
    "AIIOS_CONFIGURATION_MANAGEMENT_SERVICE_SECRETS_SERVICE_BASE_URL", "http://secrets.internal"
)
...
from app.events.configuration_events import ConfigurationCreatedEvent  # noqa: E402, F401
from app.models.configuration_profile import ConfigurationProfile  # noqa: E402
from app.models.enums import ConfigurationType, EnvironmentType, ProfileStatus  # noqa: E402
from app.repositories.configuration_audit import ConfigurationAuditRepository  # noqa: E402
from app.repositories.configuration_profile import ConfigurationProfileRepository  # noqa: E402
from app.repositories.configuration_version import ConfigurationVersionRepository  # noqa: E402
from app.services.audit import ConfigurationAuditService  # noqa: E402
from app.services.profile import ConfigurationProfileService, EventPublisher  # noqa: E402
from app.services.version import ConfigurationVersionService  # noqa: E402

UNREACHABLE_ERRORS = (OSError, TimeoutError, ConnectionError, RedisError)

SECRETS_SERVICE_BASE_URL = "http://secrets.internal"
...
def postgres_test_settings() -> DatabaseSettings:
    return DatabaseSettings(
        ...
        database_port=5433,
        database_name="aiios_configuration_management",
        database_user="aiios",
...
def redis_test_settings() -> RedisSettings:
    """Test db 12 -- distinct from every other AI-IOS service's own test db."""
...
async def make_profile(
    ...
    *,
    ...
    configuration_type: ConfigurationType = ConfigurationType.APPLICATION,
    variables: dict[str, Any] | None = None,
    target_assets: list[str] | None = None,
) -> ConfigurationProfile:
    """Create a bare :class:`ConfigurationProfile` row directly -- for
    tests exercising code paths that don't need the full
    :class:`ConfigurationProfileService.create` side-effect chain
    (version snapshot/audit/events).
    """
...
def build_version_service(db_session: AsyncSession) -> ConfigurationVersionService:
    return ConfigurationVersionService(ConfigurationVersionRepository(db_session))


def build_profile_service(
    db_session: AsyncSession, *, publish_event: EventPublisher | None = None
) -> ConfigurationProfileService:
    """Assemble a real, fully-wired :class:`ConfigurationProfileService`
    bound to *db_session* -- the shared wiring nearly every profile-
    touching service test needs, matching ``app/core/factory.py``'s own
    ...
    return ConfigurationProfileService(
        ...
        ConfigurationAuditService(ConfigurationAuditRepository(db_session)),

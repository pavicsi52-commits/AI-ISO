"""Repository for :class:`app.models.configuration_profile.ConfigurationProfile`."""
...
_SEARCHABLE_FIELDS = ("profile_name", "description")


class ConfigurationProfileRepository(BaseRepository[ConfigurationProfile]):
    """CRUD plus lookup/search/pagination for :class:`ConfigurationProfile`."""
    ...
    async def list_for_org(self, organization_id: UUID) -> list[ConfigurationProfile]:
        ...
        stmt = self._base_select().where(ConfigurationProfile.organization_id == organization_id)
    ...
    async def search_and_paginate(
        ...
        *,
        ...
        if query:
            stmt = apply_search(
                stmt, ConfigurationProfile, _SEARCHABLE_FIELDS, query, mode=SearchMode.ILIKE
            )
        if filters:
            stmt = apply_filters(stmt, ConfigurationProfile, filters)
        if sort_fields:
            stmt = apply_sorting(stmt, ConfigurationProfile, sort_fields)

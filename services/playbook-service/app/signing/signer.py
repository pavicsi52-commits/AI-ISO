...
docs/041 "SECURITY" "Support": Digital Signatures, Publisher
...
and ``shared_core.security.hashing`` only provides shared-secret HMAC
signing, neither of which is publisher-verifiable asymmetric content
signing -- so this module is built directly on ``cryptography``, the
same precedent ``services/secrets-management-service``'s own
``ssh/keygen.py`` already established for SSH keypairs in this
monorepo. Ed25519 (not RSA-PSS) is the one genuinely implemented
algorithm: modern, fast, small-signature, and the same primitive
``ssh/keygen.py`` already uses for its own ``ED25519`` case.
"""
...
def compute_fingerprint(public_key_pem: str) -> str:
    ...
    ``services/secrets-management-service``'s own
    ``ssh/keygen.compute_fingerprint`` established for SSH keys.
...
def verify_signature(checksum: str, *, signature: str, public_key_pem: str) -> bool:
    ...
    verified", the same defensive shape a publisher-verification check
    needs.
    """
    try:
        public_key = _load_public_key(public_key_pem)

"""``playbook_audit`` table -- privileged-action audit trail. Per
docs/041 "AUDIT": Creation, Modification, Approval, Publishing,
...
class PlaybookAuditEntry(BaseModel):
    """One privileged/administrative action recorded against a playbook."""

    __tablename__ = "playbook_audit"

    playbook_id: Mapped[uuid.UUID | None] = mapped_column(
        ForeignKey("playbooks.id", ondelete="CASCADE"), default=None, index=True
    )

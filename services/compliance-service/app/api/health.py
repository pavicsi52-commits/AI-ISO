...
reads or writes the compliance catalogue, and a replica taking traffic
without one would answer "no frameworks, no controls, no findings" --
which is indistinguishable from a clean bill of health and far more
dangerous than an error.

**Redis is reported but does not gate.** It backs the scheduler's
leader election; losing it makes scheduled assessments late, not
wrong.
"""
...
async def health(
    ...
    """Return the overall health of the compliance service."""
    data = HealthStatus(
        status="healthy",

"""Secrets management service settings.
...
key path, envelope-encryption master key path).
"""
...
class SecretsServiceSettings(BaseSettings):
    """Fields specific to this service, not covered by any shared_core.config section."""

    model_config = SettingsConfigDict(
        env_prefix="AIIOS_SECRETS_SERVICE_", env_file=".env", extra="ignore"
    )

    host: str = Field(default="0.0.0.0")
    port: int = Field(default=8006, ge=1, le=65_535)
    cors_allowed_origins: list[str] = Field(default_factory=list)
    jwt_public_key_path: str = Field(default="keys/jwt_public_key.pem")
    master_key_path: str = Field(default="keys/master.key")
    rotation_check_interval_seconds: int = Field(default=300, ge=1)
    lease_sweep_interval_seconds: int = Field(default=60, ge=1)
...
class Settings:
    ...
    email: EmailSettings
    service: SecretsServiceSettings


def build_settings(*, service: SecretsServiceSettings | None = None) -> Settings:
    ...
    return Settings(
        ...
        email=shared.email,
        service=service if service is not None else SecretsServiceSettings(),
    )
...
__all__ = ["SecretsServiceSettings", "Settings", "build_settings", "get_settings"]

"""HTTP tests for /health, /liveness, /readiness -- the service's OWN
process health, distinct from the org-wide connector health dashboard at
GET /integrations/health (see ``tests/test_api_analytics.py``).

None of these routes take ``organization_id`` or need ``Authorization``
headers. PostgreSQL gates readiness; Redis is reported but does not gate
(see ``app/api/health.py``'s own module docstring).
"""
...
class TestReadiness:
    ...
    async def test_readiness_reports_a_failed_cache_check_when_redis_is_unreachable(
        ...
        assert resp.status_code == HTTP_OK
        data = resp.json()["data"]
        # Redis is reported but does not gate overall status -- PostgreSQL
        # alone gates readiness (see app/api/health.py's own docstring).
        assert data["status"] == "ready"
        checks_by_name = {check["name"]: check for check in data["checks"]}

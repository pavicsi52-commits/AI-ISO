...
Recomputes every organization's rolling sync statistics from sync jobs
that already exist. **Leader-elected** through ``shared_core.scheduler``
...
class StatisticsRollupWorker:
    """Recomputes every organization's connector sync statistics."""
    ...
    async def tick(self) -> int:
        ...
        logger.info(
            "Integration hub statistics rollup complete.",
            extra={"extra_fields": {"organizations": len(organizations), "succeeded": done}},
        )
        return done

    async def _organizations(self) -> list[UUID]:
        """Every organization with at least one logged sync job."""
        async with self._session_factory() as session:
            statement = select(distinct(ConnectorSyncJob.organization_id)).limit(
    ...
    async def _recompute(self, organization_id: UUID) -> bool:
        ...
        try:
            ...
            logger.warning(
                "An integration hub statistics rollup failed; the rest of the tick continues.",
                extra={

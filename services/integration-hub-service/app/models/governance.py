"""``connector_statistics``, ``connector_reports``, and ``connector_audit``."""
...
class ConnectorStatistic(BaseModel):
    """``connector_statistics`` -- one rolled-up traffic window, org-wide."""

    __tablename__ = "connector_statistics"
    __table_args__ = (Index("ix_connector_statistics_window_start", "window_start"),)

    window_start: Mapped[datetime] = mapped_column(DateTime(timezone=True))
    window_end: Mapped[datetime] = mapped_column(DateTime(timezone=True))
    syncs_attempted: Mapped[int] = mapped_column(Integer, default=0)
    syncs_succeeded: Mapped[int] = mapped_column(Integer, default=0)
    syncs_failed: Mapped[int] = mapped_column(Integer, default=0)
    ...
    by_connector: Mapped[dict[str, int]] = mapped_column(JSON, default=dict)


class ConnectorReport(BaseModel):
    """``connector_reports`` -- one generated report."""

    __tablename__ = "connector_reports"
    __table_args__ = (Index("ix_connector_reports_kind", "kind"),)
...
class ConnectorAudit(BaseModel):
    """``connector_audit`` -- the append-only audit trail."""

    __tablename__ = "connector_audit"
    __table_args__ = (
        Index("ix_connector_audit_action", "action"),
        Index("ix_connector_audit_entity_id", "entity_id"),
        Index("ix_connector_audit_occurred_at", "occurred_at"),
    )
...
__all__ = ["ConnectorAudit", "ConnectorReport", "ConnectorStatistic"]

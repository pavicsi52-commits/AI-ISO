"""The tool registry and its executor ("TOOL CALLING").

A tool has two halves: a database row (:class:`~app.models.ai_tool
.AiTool`) describing it to the model and carrying its permission, and a
Python handler that actually does the work. This module binds them and
enforces the rules before anything runs.

**Authorization is structural, layered, and fails closed:**

1. The agent must list the tool in its own ``tool_keys``. A model
   cannot invoke a tool its agent was never granted, no matter what it
   emits.
2. The tool must be enabled.
3. If the tool declares ``required_permission``, the caller must hold
   it.
4. A mutating tool additionally requires the caller to have explicitly
   opted into mutations for that turn -- so an assistant answering a
   question can never restart a server as a side effect.

Every attempt is recorded either way. A denial is a first-class
outcome (:attr:`~app.models.enums.ToolCallStatus.DENIED`) with its own
reason, not a silent no-op, which is what makes "Permission-aware Tool
Calls" auditable after the fact.
"""
...
def authorize(
    ...
    *,
    ...
    caller_permissions: Sequence[str],
    allow_mutating: bool,
    ...
    if tool.required_permission and tool.required_permission not in caller_permissions:
        return AuthorizationDecision(
            allowed=False,
            reason=(
                f"Caller lacks permission {tool.required_permission!r} "
...
def validate_arguments(tool: AiTool, arguments: dict[str, Any]) -> str | None:
...
def to_specification(tool: AiTool) -> ToolSpecification:

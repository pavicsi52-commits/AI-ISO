...
Recomputes every organization's rolling marketplace activity statistics
from plugins/installations/reviews that already exist. **Leader-elected**
...
class StatisticsRollupWorker:
    """Recomputes every organization's plugin marketplace statistics."""
    ...
    async def tick(self) -> int:
        ...
        logger.info(
            "Plugin marketplace statistics rollup complete.",
            extra={"extra_fields": {"organizations": len(organizations), "succeeded": done}},
        )
        return done

    async def _organizations(self) -> list[UUID]:
        """Every organization with at least one registered plugin."""
        async with self._session_factory() as session:
            statement = select(distinct(Plugin.organization_id)).limit(
    ...
    async def _recompute(self, organization_id: UUID) -> bool:
        ...
        try:
            ...
            logger.warning(
                "A plugin marketplace statistics rollup failed; the rest of the tick continues.",
                extra={

...
**PostgreSQL gates readiness.** Every scheduling decision this service
makes reads or writes a job definition, its schedule, or its execution
history, and a replica taking traffic without a database would answer
"nothing is due" -- indistinguishable from a genuinely empty queue and
far more dangerous than an error.

**Redis is reported but does not gate.** It backs the scheduler's
leader election; losing it makes the due-schedule, retry, statistics,
and maintenance sweeps late, not wrong.
"""
...
async def health(
    ...
    """Return the overall health of the scheduler service."""
    data = HealthStatus(
        status="healthy",
...
async def readiness(request: Request) -> SuccessResponse[ReadinessStatus]:
    ...
    # sweeps late, rather than making its answers wrong.

...
async def get_dashboard(
    ...
    """The snapshot a dashboard reads on load."""
...
@router.get("/scheduler/reports/{report_id}/download", summary="Download a report's content")
async def download_report(organization_id: UUID, report_id: UUID, reports: ReportSvc) -> Any:
    ...
    route to ``PlainTextResponse`` 500s the one format this endpoint's
    own default is supposed to support, a lesson carried forward from
    ``services/change-management-service``'s own copy of this route.

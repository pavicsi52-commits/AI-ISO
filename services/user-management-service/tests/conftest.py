"""Shared fixtures for the user management service's test suite.

Everything here runs against real infrastructure (the repository root's
docker-compose Postgres/Redis/RabbitMQ/MinIO) -- the same discipline
``services/authentication-service/tests/conftest.py`` established.
...
This service's own database is physically separate from
``services/authentication-service``'s (``aiios_user_management`` vs
``aiios``) -- both services define their own ``users`` table and a
single shared Postgres database can't hold two same-named tables in
one schema. See the package README's "Architecture" section.
"""
...
# already set the same way in services/authentication-service's own conftest.
os.environ.setdefault("AIIOS_DATABASE_HOST", "localhost")
os.environ.setdefault("AIIOS_DATABASE_PORT", "5433")
os.environ.setdefault("AIIOS_DATABASE_NAME", "aiios_user_management")
...
os.environ.setdefault("AIIOS_REDIS_DB", "4")
...
_TEST_KEY_DIR = Path(tempfile.gettempdir()) / "aiios_ums_test_keys"
...
os.environ.setdefault("AIIOS_USER_SERVICE_JWT_PUBLIC_KEY_PATH", str(_TEST_PUBLIC_KEY_PATH))
...
TEST_BUCKET = "test-user-management"
...
def postgres_test_settings() -> DatabaseSettings:
    return DatabaseSettings(
        ...
        database_port=5433,
        database_name="aiios_user_management",
        database_user="aiios",
...
def redis_test_settings() -> RedisSettings:
    """Test db 4 -- distinct from the auth service's own test db 3."""

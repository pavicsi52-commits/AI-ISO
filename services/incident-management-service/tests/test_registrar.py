...
async def scheduler() -> AsyncIterator[SchedulerManager]:
    ...
    manager = create_scheduler_framework(
        queue.manager, cache.client, queue_name="incident_management_test_queue"
    )
    yield manager
...
class TestJobRegistration:
    async def test_all_four_jobs_register_under_deterministic_ids(
        ...
        register_sla_sweep(scheduler, _noop, interval_seconds=60)
        register_escalation_sweep(scheduler, _noop, interval_seconds=60)
        ...
        assert scheduler.registry.get(SLA_SWEEP_JOB_ID).job_type is JobType.SYSTEM
        assert scheduler.registry.get(ESCALATION_SWEEP_JOB_ID).job_type is JobType.SYSTEM
    ...
    async def test_a_schedule_carries_its_interval(self, scheduler: SchedulerManager) -> None:
        ...
        # background job.
        register_sla_sweep(scheduler, _noop, interval_seconds=60)
        job = scheduler.registry.get(SLA_SWEEP_JOB_ID)
    ...
    async def test_a_non_positive_interval_is_refused(
        ...
        with pytest.raises(ValueError, match="must be positive"):
            register_sla_sweep(scheduler, _noop, interval_seconds=interval)
        with pytest.raises(ValueError, match="must be positive"):
            register_escalation_sweep(scheduler, _noop, interval_seconds=interval)
        with pytest.raises(ValueError, match="must be positive"):

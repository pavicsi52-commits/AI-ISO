"""``incident_reports``, ``incident_statistics``, ``incident_audit``.

The three tables that turn incident history into something a leadership
...
class IncidentReport(BaseModel):
    """``incident_reports`` -- a generated document."""

    __tablename__ = "incident_reports"
    __table_args__ = (Index("ix_incident_report_org", "organization_id", "generated_at"),)
...
class IncidentStatistic(BaseModel):
    """``incident_statistics`` -- one rolled-up window."""

    __tablename__ = "incident_statistics"
    __table_args__ = (Index("ix_incident_statistic_org", "organization_id", "window_start"),)
    ...
    incidents_closed: Mapped[int] = mapped_column(Integer, default=0)
    incidents_reopened: Mapped[int] = mapped_column(Integer, default=0)
    major_incidents: Mapped[int] = mapped_column(Integer, default=0)
    open_total: Mapped[int] = mapped_column(Integer, default=0)

    avg_mtta_seconds: Mapped[float | None] = mapped_column(Float, default=None)
    avg_mttr_seconds: Mapped[float | None] = mapped_column(Float, default=None)
    p90_mttr_seconds: Mapped[float | None] = mapped_column(Float, default=None)
    """The 90th percentile alongside the average, deliberately. An
    average MTTR can look healthy while one incident in ten drags on for
    days -- the number a percentile exists to surface and an average
    quietly launders."""

    sla_met_count: Mapped[int] = mapped_column(Integer, default=0)
    sla_breached_count: Mapped[int] = mapped_column(Integer, default=0)
    sla_compliance_rate: Mapped[float] = mapped_column(Float, default=0.0)

    escalations_triggered: Mapped[int] = mapped_column(Integer, default=0)
    recurring_incidents: Mapped[int] = mapped_column(Integer, default=0)
    automation_success_count: Mapped[int] = mapped_column(Integer, default=0)
    automation_attempt_count: Mapped[int] = mapped_column(Integer, default=0)
...
class IncidentAudit(BaseModel):
    """``incident_audit`` -- append-only record of who did what.
    ...
    __table_args__ = (
        Index("ix_incident_audit_org", "organization_id", "occurred_at"),
        Index("ix_incident_audit_action", "organization_id", "action"),
        Index("ix_incident_audit_actor", "organization_id", "actor_id"),
    )
...
__all__ = ["IncidentAudit", "IncidentReport", "IncidentStatistic"]

...
reads or writes the incident catalogue, and a replica taking traffic
without one would answer "no open incidents" -- indistinguishable from
a genuinely quiet estate and far more dangerous than an error.

**Redis is reported but does not gate.** It backs the scheduler's
leader election; losing it makes the SLA and escalation sweeps late,
not wrong.
"""
...
async def health(
    ...
    """Return the overall health of the incident management service."""
    data = HealthStatus(
        status="healthy",

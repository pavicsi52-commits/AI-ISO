...
class NotificationChannelConfigRepository(BaseRepository[NotificationChannelConfig]):
    ...
    async def require_in_org(self, organization_id: UUID, config_id: UUID) -> NotificationChannelConfig:
        ...
        if found is None:
            raise NotFoundError(f"No channel configuration with id {config_id} in this organization.")
        return found

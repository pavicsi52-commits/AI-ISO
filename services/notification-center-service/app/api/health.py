...
**PostgreSQL gates readiness.** Every notification this service creates,
routes, renders, and delivers reads or writes its own tables; a replica
taking traffic without a database would silently drop every notification
request rather than fail loudly.

**Redis is reported but does not gate.** It backs the retry, digest,
statistics, and announcement-expiry sweeps' leader election; losing it
makes those sweeps late, not wrong.
"""
...
async def health(
    ...
    """Return the overall health of the notification center service."""
    data = HealthStatus(
        status="healthy",

...
notifications and deliveries that already exist. **Leader-elected**
...
class StatisticsWorker:
    """Recomputes every organization's notification statistics."""
    ...
    async def tick(self) -> int:
        ...
        logger.info(
            "Notification statistics rollup complete.",
            extra={"extra_fields": {"organizations": len(organizations), "succeeded": done}},
        )
        return done

    async def _organizations(self) -> list[UUID]:
        """Every organization with at least one notification."""
        async with self._session_factory() as session:
            statement = select(distinct(Notification.organization_id)).limit(self._max_per_tick)

...
async def scheduler() -> AsyncIterator[SchedulerManager]:
    ...
    manager = create_scheduler_framework(
        queue.manager, cache.client, queue_name="notification_service_test_queue"
    )
    yield manager
...
class TestJobRegistration:
    async def test_all_four_jobs_register_under_deterministic_ids(
        ...
        register_digest_sweep(scheduler, _noop, interval_seconds=300)
        register_statistics_rollup(scheduler, _noop, interval_seconds=900)
        register_announcement_expiry_sweep(scheduler, _noop, interval_seconds=300)

        assert scheduler.registry.get(RETRY_SWEEP_JOB_ID).job_type is JobType.SYSTEM
        assert scheduler.registry.get(DIGEST_SWEEP_JOB_ID).job_type is JobType.SYSTEM
        assert scheduler.registry.get(STATISTICS_ROLLUP_JOB_ID).job_type is JobType.SYSTEM
        assert scheduler.registry.get(ANNOUNCEMENT_EXPIRY_SWEEP_JOB_ID).job_type is JobType.SYSTEM
    ...
    async def test_registration_computes_a_first_due_time(
        ...
        job = register_announcement_expiry_sweep(scheduler, _noop, interval_seconds=300)
    ...
    async def test_each_jobs_metadata_names_its_own_component(
        ...
        assert scheduler.registry.get(DIGEST_SWEEP_JOB_ID).metadata["component"] == (
            "notification-digest-sweep"
        )
        assert scheduler.registry.get(STATISTICS_ROLLUP_JOB_ID).metadata["component"] == (
            "notification-statistics"
        )
        assert scheduler.registry.get(ANNOUNCEMENT_EXPIRY_SWEEP_JOB_ID).metadata["component"] == (
            "notification-announcement-expiry"
        )
    ...
    async def test_a_non_positive_interval_is_refused(
        ...
        with pytest.raises(ValueError, match="must be positive"):
            register_digest_sweep(scheduler, _noop, interval_seconds=interval)
        with pytest.raises(ValueError, match="must be positive"):
            register_statistics_rollup(scheduler, _noop, interval_seconds=interval)
        with pytest.raises(ValueError, match="must be positive"):
            register_announcement_expiry_sweep(scheduler, _noop, interval_seconds=interval)

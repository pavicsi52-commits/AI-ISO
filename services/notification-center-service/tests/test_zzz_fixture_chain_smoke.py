...
class TestSmoke:
    async def test_create_and_dispatch_in_app_succeeds(
    ...
    async def test_template_create_and_preview(
        self, make_template, template_service, organization_id
    ):
    ...
    async def test_subscription_and_broadcast(
        ...
        await subscription_service.subscribe(
            organization_id, "user-3", SubscriptionKind.TOPIC, "outages"
        )

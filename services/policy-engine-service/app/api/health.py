...
**PostgreSQL gates readiness here, unlike in most services.** This one
answers every protected operation on the platform, and it cannot
decide anything without its policy catalogue -- a replica that took
traffic with no database would fail closed on every request, which is
safe but indistinguishable from an outage. Better to leave the
rotation and let a healthy replica answer.

**Redis is reported but does not gate.** It backs decision caching and
the scheduler's leader election; losing it makes this service slower
and its rollups late, not wrong.
"""
...
async def health(
    ...
    """Return the overall health of the policy engine service."""
    data = HealthStatus(
        status="healthy",
...
async def readiness(request: Request) -> SuccessResponse[ReadinessStatus]:
    ...
    # Non-gating: a cache outage slows this service down and makes its
    # rollups late, rather than making its answers wrong.

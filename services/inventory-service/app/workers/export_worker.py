"""Background worker: bulk asset export processing.
...
EXPORT_QUEUE_NAME = "inventory_export_queue"

ExportServiceFactory = Callable[[], AbstractAsyncContextManager[AssetExportService]]
...
def build_export_worker(service_factory: ExportServiceFactory) -> MessageHandler:
    ...
    async def handle_export_job(message: QueueMessage) -> None:
        ...
        try:
            ...
            logger.exception(
                "Asset export job failed.", extra={"extra_fields": {"job_id": str(job_id)}}
            )
            raise

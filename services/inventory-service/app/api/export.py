"""``POST /inventory/export``, plus the job-status endpoint docs/036's
own "EXPORT" functional section requires ("Background Processing"
implies polling) but its literal REST list doesn't separately enumerate.
"""
...
router = APIRouter(prefix="/inventory/export", tags=["Export"])
...
async def _to_response(job: AssetExportJob, exports: ExportSvc) -> ExportJobResponse:
...
def _filter_criteria(body: ExportRequest) -> dict[str, Any]:
    ...
    if body.asset_type is not None:
        criteria["asset_type"] = body.asset_type.value
...
async def start_export(
    ...
    """Queue a bulk asset-export job ("Background Processing")."""
    job = await exports.create_job(
        current_user_id,
        organization_id=body.organization_id,
        target_format=body.target_format,
        filter_criteria=_filter_criteria(body),
    )

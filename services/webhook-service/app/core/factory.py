...
Redis, events, the outbound HTTP client deliveries forward through, the
...
async def _build_workers(
    ...
    All five are leader-elected: each is pure database (plus, for the
    retry/replay sweeps, outbound HTTP) work with no per-replica state,
    ...
    manager = create_scheduler_framework(
        queue.manager, redis_client, queue_name="webhook_service_scheduler_queue"
    )
    register_retry_sweep(
        manager,
        RetrySweepWorker(
            session_factory,
            http_client,
            encryption_key=settings.service.secret_encryption_key,
    ...
    register_replay_processor(
        manager,
        ReplayProcessorWorker(
            session_factory, http_client, encryption_key=settings.service.secret_encryption_key
        ).run_job,
        interval_seconds=settings.service.replay_processor_sweep_seconds,
    )
    register_statistics_rollup(
        manager,
    ...
    register_idempotency_expiry_sweep(
        manager,
        IdempotencyExpirySweepWorker(session_factory).run_job,
        interval_seconds=settings.service.idempotency_expiry_sweep_seconds,
    )
    register_secret_expiry_sweep(
        manager,
        SecretExpirySweepWorker(
            session_factory, encryption_key=settings.service.secret_encryption_key
        ).run_job,
        interval_seconds=settings.service.secret_expiry_sweep_seconds,
    )
    await manager.start()
    return manager
...
async def _lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    app.state.http_client = httpx.AsyncClient(
        limits=httpx.Limits(
            max_connections=settings.service.delivery_max_connections,
            max_keepalive_connections=settings.service.delivery_max_keepalive_connections,
        ),
        timeout=httpx.Timeout(
            connect=settings.service.delivery_connect_timeout_seconds,
            read=settings.service.delivery_read_timeout_seconds,
            write=settings.service.delivery_read_timeout_seconds,
            pool=settings.service.delivery_connect_timeout_seconds,
        ),
    )
    ...
    logger.info(
        "webhook-service starting up",
        extra={
            "extra_fields": {
                "workers_enabled": settings.service.workers_enabled,
                "retry_sweep_seconds": settings.service.retry_sweep_seconds,
                "replay_processor_sweep_seconds": settings.service.replay_processor_sweep_seconds,
...
def create_app() -> FastAPI:
    ...
    app = FastAPI(
        title="AI-IOS Enterprise Webhook Service",
        description=(
            "Centralized reception, filtering, transformation, signing, "
            "delivery, retry, replay, tracking, and audit for webhooks "
            "across AI-IOS -- incoming partner webhooks, outgoing "
            "subscriber deliveries, and internal platform events alike. "

...
Recomputes every organization's rolling delivery statistics from
attempts that already exist. **Leader-elected** through
...
class StatisticsRollupWorker:
    """Recomputes every organization's webhook delivery statistics."""
    ...
    async def tick(self) -> int:
        ...
        logger.info(
            "Webhook statistics rollup complete.",
            extra={"extra_fields": {"organizations": len(organizations), "succeeded": done}},
        )
        return done

    async def _organizations(self) -> list[UUID]:
        """Every organization with at least one logged delivery attempt."""
        async with self._session_factory() as session:
            statement = select(distinct(WebhookDeliveryAttempt.organization_id)).limit(
    ...
    async def _recompute(self, organization_id: UUID) -> bool:
        ...
        try:
            ...
            logger.warning(
                "A webhook statistics rollup failed; the rest of the tick continues.",
                extra={

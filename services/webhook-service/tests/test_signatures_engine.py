...
class TestSignPayload:
    def test_sha256_matches_a_hand_computed_digest(self) -> None:
        ...
        assert sign_payload(body, secret="secret", algorithm=SignatureAlgorithm.HMAC_SHA256) == expected

    def test_sha512_matches_a_hand_computed_digest(self) -> None:
        body = b'{"hello":"world"}'
        expected = hmac.new(b"secret", body, hashlib.sha512).hexdigest()
        assert sign_payload(body, secret="secret", algorithm=SignatureAlgorithm.HMAC_SHA512) == expected
...
class TestVerifyWithAnySecret:
    ...
    def test_a_signature_from_an_older_secret_still_verifies_during_rotation(self) -> None:
        body = b"payload"
        old = SigningSecret(version=1, secret="old-secret", algorithm=SignatureAlgorithm.HMAC_SHA256)
        new = SigningSecret(version=2, secret="new-secret", algorithm=SignatureAlgorithm.HMAC_SHA256)
...
class TestBuildSignedHeaders:
    ...
    def test_signature_covers_timestamp_and_nonce_together_with_the_body(self) -> None:
        ...
        body = b"body"
        headers = build_signed_headers(
            body, secret="secret", algorithm=SignatureAlgorithm.HMAC_SHA256, timestamp=timestamp, nonce=nonce
        )
...
class TestVerifySignedRequest:
    ...
    def test_a_fresh_correctly_signed_request_returns_the_matching_secret(self) -> None:
        ...
        headers = self._headers_for(
            body=body, secret="s1", timestamp=1000, nonce="n1", algorithm=SignatureAlgorithm.HMAC_SHA256
        )
    ...
    def test_rotation_still_verifies_against_an_older_secret(self) -> None:
        ...
        headers = self._headers_for(
            body=body, secret="old", timestamp=1000, nonce="n1", algorithm=SignatureAlgorithm.HMAC_SHA256
        )
        result = verify_signed_request(
            body,
            signature=headers["X-Webhook-Signature"],

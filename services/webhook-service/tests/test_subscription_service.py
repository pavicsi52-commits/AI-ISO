...
class TestGet:
    async def test_returns_the_matching_subscription(
        self, subscription_service: SubscriptionService, organization_id: uuid.UUID, make_endpoint, make_subscription
    ) -> None:
        endpoint = await make_endpoint()
...
class TestResolveMatching:
    ...
    async def test_an_event_scoped_subscription_matches_by_glob_pattern(
        ...
        non_matching_event = EventContext(event_type="invoice.created", organization_id=str(organization_id))
        assert await subscription_service.resolve_matching(organization_id, non_matching_event) == []
    ...
    async def test_event_types_narrows_a_wildcard_subscription(
        ...
        matching_event = EventContext(event_type="order.created", organization_id=str(organization_id))
        assert [one.id for one in await subscription_service.resolve_matching(organization_id, matching_event)] == [
            created.id
        ]

        other_event = EventContext(event_type="invoice.created", organization_id=str(organization_id))
...
class TestWebhookSubscriptionRepository:
    ...
    async def test_list_for_endpoint_scopes_by_endpoint(
        self, subscriptions_repo: WebhookSubscriptionRepository, organization_id: uuid.UUID, make_endpoint, make_subscription
    ) -> None:

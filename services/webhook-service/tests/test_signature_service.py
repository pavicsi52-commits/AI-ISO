...
class TestCreate:
    ...
    async def test_a_second_create_call_increments_the_version(
        ...
        endpoint = await make_endpoint()
        await signature_service.create(organization_id, endpoint_id=endpoint.id, secret="first-secret12")
...
class TestRotate:
    ...
    async def test_rotating_moves_the_previous_active_secret_to_rotating(
        ...
        rotated = await signature_service.rotate(
            organization_id, endpoint_id=endpoint.id, new_secret="rotated-secret12", overlap_hours=24
        )
    ...
    async def test_repeated_rotation_assigns_strictly_sequential_versions(
        ...
        second = await signature_service.rotate(
            organization_id, endpoint_id=endpoint.id, new_secret="secret-number-two", overlap_hours=0
        )
        assert second.secret_version == 2
        ...
        third = await signature_service.rotate(
            organization_id, endpoint_id=endpoint.id, new_secret="secret-number-three", overlap_hours=0
        )
        assert third.secret_version == 3

    async def test_the_rotated_secret_keeps_the_previous_algorithm(
        self, signature_service: SignatureService, organization_id: uuid.UUID, make_endpoint, make_signature
    ) -> None:
        endpoint = await make_endpoint()
        await make_signature(endpoint.id, secret="original-secret1", algorithm=SignatureAlgorithm.HMAC_SHA512)
        rotated = await signature_service.rotate(
...
class TestExpireDue:
    async def test_expires_a_rotating_secret_past_its_overlap(
        ...
        expired_count = await signature_service.expire_due(now=datetime.now(UTC) + timedelta(seconds=1))
    ...
    async def test_respects_the_limit(
        ...
        await signature_service.rotate(
            organization_id, endpoint_id=endpoint_a.id, new_secret="secret-a-value2", overlap_hours=0
        )
        await signature_service.rotate(
            organization_id, endpoint_id=endpoint_b.id, new_secret="secret-b-value2", overlap_hours=0
        )

        expired_count = await signature_service.expire_due(now=datetime.now(UTC) + timedelta(seconds=1), limit=1)
        assert expired_count == 1
...
class TestWebhookSignatureRepository:
    ...
    async def test_list_rotating_due_for_expiry_is_unscoped_by_organization(
        ...
        due = await signatures_repo.list_rotating_due_for_expiry(now=datetime.now(UTC) + timedelta(seconds=1))
        assert len(due) == 1

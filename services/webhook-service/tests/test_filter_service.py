...
class TestCreate:
    async def test_creates_with_defaults(
        self, filter_service: FilterService, organization_id: uuid.UUID, make_endpoint, make_subscription
    ) -> None:
...
class TestListForSubscription:
    ...
    async def test_scoped_by_subscription(
        ...
        await filter_service.create(organization_id, subscription_id=subscription_b.id, name="f-b", rules=[])
...
class TestPasses:
    ...
    async def test_all_mode_requires_every_rule_to_match(
        ...
        assert await filter_service.passes(subscription.id, {"severity": "critical", "status": "open"}) is True
        assert (
            await filter_service.passes(subscription.id, {"severity": "critical", "status": "closed"}) is False
        )
    ...
    async def test_any_mode_requires_only_one_rule_to_match(
        ...
        assert await filter_service.passes(subscription.id, {"severity": "info", "status": "open"}) is True
        assert (
            await filter_service.passes(subscription.id, {"severity": "info", "status": "closed"}) is False
        )

...
from app.subscriptions.engine import EventContext, SubscriptionCandidate, matches, matching_subscriptions
...
class TestSubscriptionCandidateDefaults:
    def test_event_types_defaults_to_an_empty_tuple(self) -> None:
        candidate = SubscriptionCandidate(id="s1", scope=SubscriptionScope.WILDCARD, scope_reference=None)
...
class TestEventTypeMatching:
    ...
    def test_no_matching_pattern_among_many(self) -> None:
        candidate = _candidate(scope=SubscriptionScope.WILDCARD, event_types=("user.*", "billing.*"))

...
from tests.conftest import FAKE_BACKEND_URL, HTTP_CREATED, HTTP_NOT_FOUND, HTTP_OK, HTTP_UNAUTHORIZED
...
class TestCreateSignature:
    async def test_create_needs_no_auth(
        ...
        endpoint_id = await _create_endpoint(client, auth_headers, organization_id, "noauth-create-sig-endpoint")
        resp = await client.post(
            "/webhooks/signatures",
            params={"organization_id": str(organization_id)},
    ...
    async def test_create_returns_the_new_signature(
        ...
        endpoint_id = await _create_endpoint(client, auth_headers, organization_id, "create-sig-endpoint")
        resp = await client.post(
            "/webhooks/signatures",
            params={"organization_id": str(organization_id)},
            json={
    ...
    async def test_create_rejects_a_secret_shorter_than_eight_characters(
        ...
        endpoint_id = await _create_endpoint(client, auth_headers, organization_id, "short-secret-endpoint")
        resp = await client.post(
            "/webhooks/signatures",
            params={"organization_id": str(organization_id)},
...
class TestRotateSignature:
    async def test_rotate_requires_auth(
        ...
        endpoint_id = await _create_endpoint(client, auth_headers, organization_id, "noauth-rotate-sig-endpoint")
        await client.post(
            "/webhooks/signatures",
            params={"organization_id": str(organization_id)},
    ...
    async def test_rotate_returns_the_new_active_secret(
        ...
        endpoint_id = await _create_endpoint(client, auth_headers, organization_id, "rotate-sig-endpoint")
        await client.post(
            "/webhooks/signatures",
            params={"organization_id": str(organization_id)},
            json={"endpoint_id": endpoint_id, "secret": "original-secret1"},
        ...
        resp = await client.put(
            ...
            params={"organization_id": str(organization_id)},
            headers=auth_headers(uuid.uuid4()),
            json={"endpoint_id": endpoint_id, "new_secret": "rotated-secret12", "overlap_hours": 12},
        )
    ...
    async def test_rotate_records_an_audit_entry(
        ...
        endpoint_id = await _create_endpoint(client, auth_headers, organization_id, "audit-rotate-sig-endpoint")
        await client.post(
            "/webhooks/signatures",
            params={"organization_id": str(organization_id)},
            json={"endpoint_id": endpoint_id, "secret": "original-secret1"},
...
class TestTenantIsolation:
    ...
    async def test_create_rejects_an_endpoint_from_another_organization(
        ...
        endpoint_id = await _create_endpoint(client, auth_headers, organization_id, "victim-create-sig-endpoint")
    ...
    async def test_rotate_rejects_an_endpoint_from_another_organization(
        ...
        endpoint_id = await _create_endpoint(client, auth_headers, organization_id, "victim-rotate-sig-endpoint")
        await client.post(
            "/webhooks/signatures",
            params={"organization_id": str(organization_id)},
            json={"endpoint_id": endpoint_id, "secret": "original-secret1"},

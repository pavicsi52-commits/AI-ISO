...
PostgreSQL, which gates it, plus Redis and Neo4j, which do not.
"""
...
async def health(
    ...
    """Return the overall health of the AI agent platform service."""
    data = HealthStatus(
        status="healthy",
...
async def _graph_check(request: Request) -> ReadinessCheck:
    """Whether the knowledge graph answers, when configured.

    Never gates readiness -- the Knowledge Graph Query tool kind and
    reasoning mode are optional capabilities, not core to serving
    traffic.
    """
    graph_client = getattr(request.app.state, "graph_client", None)
    if graph_client is None or not graph_client.enabled:
        return ReadinessCheck(name="graph", status="ok", detail="not configured on this deployment")
    if await graph_client.verify():
        return ReadinessCheck(name="graph", status="ok", detail="neo4j responded")
    return ReadinessCheck(name="graph", status="failed", detail="neo4j did not respond")
...
async def readiness(request: Request) -> SuccessResponse[ReadinessStatus]:
    ...
    checks.append(await _graph_check(request))

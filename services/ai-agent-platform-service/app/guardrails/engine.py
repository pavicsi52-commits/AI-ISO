"""The guardrail pipeline (docs/060 "GUARDRAILS").
...
- :func:`screen_user_input` -- prompt-injection detection on anything a
  caller sends into an agent task.
- :func:`screen_retrieved_context` -- **injection detection plus
  redaction on retrieved content**. This is the one most often skipped
  and the one that matters most: a tool result or knowledge-graph
  fact is untrusted input the moment its text enters an agent's
  context, and an attacker who can influence that source can otherwise
  plant instructions the agent will follow.
- :func:`screen_model_output` -- redaction before anything reaches the
  caller, so a secret that slipped into context cannot be echoed back.
...
def screen_user_input(text: str) -> GuardrailResult:
    """Screen text a caller sent into an agent task.

    A detection **blocks**: the caller is a real party who can be told
...
def screen_retrieved_context(text: str) -> GuardrailResult:
    """Screen a passage retrieved by a tool call (Knowledge Graph query,
    REST fetch, database read, ...).

    A detection **neutralises rather than blocks**: dropping the whole
    passage would let anyone who can influence one data source delete
    that source from the agent's context simply by writing an
    injection string into it. The directive is replaced, the rest of
    the legitimate content survives, and the finding is recorded.

    Redaction runs regardless, since retrieved content routinely
    contains credentials that must never reach the model.
    """
    findings = detect_injection(text)
    working = text
    for _name, pattern in _INJECTION_PATTERNS:
        working = pattern.sub("[REMOVED: instruction-like text in retrieved content]", working)
...
def screen_model_output(text: str) -> GuardrailResult:
    """Screen a model's own reply before it reaches the caller or is
    persisted as an execution result.
    """

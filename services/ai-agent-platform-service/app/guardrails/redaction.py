"""Secret and PII redaction (docs/060 "GUARDRAILS": PII Detection, Secret
Redaction).

Runs over **both** directions: text retrieved by a tool before it is
shown to the model, and the model's own output before it reaches the
caller. Redacting only one direction would still leak -- a secret
pulled from a tool result into context can be echoed straight back.

    ...
    async def chat(
        ...
        *,
        ...
        try:
            ...
            raise AIError(f"google_gemini is unreachable: {exc}") from exc
        latency_ms = (time.perf_counter() - started) * 1_000
        if response.status_code != httpx.codes.OK:
            raise AIError(
                f"google_gemini returned HTTP {response.status_code} for model {model!r}."
            )

        body = response.json()
        candidates = body.get("candidates") or []
        if not candidates:
            raise AIError(f"google_gemini returned no candidates for model {model!r}.")

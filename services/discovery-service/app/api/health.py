...
``services/inventory-service/app/api/health.py``, minus the Neo4j check
(this service never opens a Neo4j driver of its own -- see
``app/config/settings.py``'s own module docstring).
"""
...
async def health(
    ...
    """Return the overall health of the discovery service."""
    data = HealthStatus(
        status="healthy",

"""Shared fixtures for the automation service's test suite.
...
No Neo4j fixture exists here -- docs/040 names no graph concept for
this service.

**Runner/connector calls**: shell/bash/python/powershell/ansible
runners are genuinely exercised via real ``asyncio.create_subprocess_exec``
calls against the host's own interpreters -- no mocking. The SSH
connector is live-tested against a real, locally run OpenSSH Docker
container, matching Gitea's role in
``services/configuration-management-service``'s own test suite.
Secrets/Inventory/Configuration Management cross-service HTTP calls use
``pytest-httpx`` against their real documented response shapes, never a
live account.

This service's own database (``aiios_automation``) is physically
separate from every other AI-IOS service's. Redis test db 13 --
...
os.environ.setdefault("AIIOS_DATABASE_NAME", "aiios_automation")
...
os.environ.setdefault("AIIOS_REDIS_DB", "13")
...
_TEST_KEY_DIR = Path(tempfile.gettempdir()) / "aiios_automation_test_keys"
...
os.environ.setdefault("AIIOS_AUTOMATION_SERVICE_JWT_PUBLIC_KEY_PATH", str(_TEST_PUBLIC_KEY_PATH))
os.environ.setdefault(
    "AIIOS_AUTOMATION_SERVICE_SECRETS_SERVICE_BASE_URL", "http://secrets.internal"
)
os.environ.setdefault(
    "AIIOS_AUTOMATION_SERVICE_INVENTORY_SERVICE_BASE_URL", "http://inventory.internal"
)
os.environ.setdefault(
    "AIIOS_AUTOMATION_SERVICE_CONFIGURATION_SERVICE_BASE_URL", "http://configuration.internal"
)
...
from app.events.automation_events import AutomationCreatedEvent  # noqa: E402, F401
from app.models.automation_job import AutomationJob  # noqa: E402
from app.models.automation_target import AutomationTarget  # noqa: E402
...
from app.repositories.automation_audit import AutomationAuditRepository  # noqa: E402
from app.repositories.automation_job import AutomationJobRepository  # noqa: E402
from app.services.audit import AutomationAuditService  # noqa: E402
from app.services.job import AutomationJobService, EventPublisher  # noqa: E402
...
def postgres_test_settings() -> DatabaseSettings:
    return DatabaseSettings(
        ...
        database_port=5433,
        database_name="aiios_automation",
        database_user="aiios",
...
def redis_test_settings() -> RedisSettings:
    """Test db 13 -- distinct from every other AI-IOS service's own test db."""
...
async def make_job(
    ...
    *,
    ...
    automation_type: AutomationType = AutomationType.CUSTOM_AUTOMATION,
    playbook_type: PlaybookType = PlaybookType.SHELL_SCRIPT,
    execution_mode: ExecutionMode = ExecutionMode.MANUAL,
    content: str = "echo hello",
    variables: dict[str, Any] | None = None,
    timeout_seconds: int | None = None,
) -> AutomationJob:
    """Create a bare :class:`AutomationJob` row directly -- for tests
    exercising code paths that don't need the full
    :class:`AutomationJobService.create` side-effect chain (audit/events).
    """
    job = AutomationJob(
...
async def make_target(
    ...
    *,
    ...
    target_type: ExecutionTargetType = ExecutionTargetType.PHYSICAL_SERVER,
    connector_type: ConnectorType = ConnectorType.SSH,
    address: str = "127.0.0.1",
    port: int | None = 22,
    username: str | None = "root",
    credential_ref: str | None = None,
) -> AutomationTarget:
    """Create a bare :class:`AutomationTarget` row directly."""
...
def build_audit_service(db_session: AsyncSession) -> AutomationAuditService:
...
def build_job_service(
    ...
    """Assemble a real, fully-wired :class:`AutomationJobService` bound
    to *db_session* -- the shared wiring nearly every job-touching

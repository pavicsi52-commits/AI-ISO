...
database and Neo4j this service depends on -- matches
``services/inventory-service/app/api/health.py`` exactly (this service
has no Redis/RabbitMQ readiness dependency of its own since it has no
worker/queue integration, unlike that service).
"""
...
async def health(
    ...
    """Return the overall health of the asset management service."""
    data = HealthStatus(
        status="healthy",
...
async def readiness(request: Request) -> SuccessResponse[ReadinessStatus]:
    ...
    if neo4j_driver is not None:
        try:
            ...
            statuses.append(HealthStatusEnum.HEALTHY)
            checks.append(ReadinessCheck(name="neo4j", status="ok", detail=None))
        except Exception as exc:
            statuses.append(HealthStatusEnum.UNHEALTHY)
            checks.append(ReadinessCheck(name="neo4j", status="failed", detail=str(exc)))

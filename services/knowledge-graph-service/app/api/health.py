...
deliberate: several endpoints -- sync history, audit, saved queries,
statistics read-back -- answer entirely from PostgreSQL, and taking the
service out of rotation would make an operator investigating a Neo4j
outage lose the tooling that tells them about it.
"""
...
async def health(
    ...
    """Return the overall health of the knowledge graph service."""
    data = HealthStatus(
        status="healthy",
    ...
    if client is not None:
        ...
        checks.append(
            ReadinessCheck(
                ...
                detail=(
                    f"neo4j database {client.database}"
                    if reachable
                    else "neo4j is unreachable; graph endpoints will fail"
                ),
            )
        )

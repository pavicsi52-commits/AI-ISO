...
class TestInventoryClient:
    ...
    async def test_get_topology_returns_nodes(
        self, httpx_mock: HTTPXMock, inventory_client: InventoryClient
    ) -> None:
        asset_id = uuid.uuid4()
        httpx_mock.add_response(
        ...
        topology = await inventory_client.get_topology(asset_id)
        assert len(topology["nodes"]) == 1

    async def test_get_topology_failure_raises(
        self, httpx_mock: HTTPXMock, inventory_client: InventoryClient
    ) -> None:
        asset_id = uuid.uuid4()
        httpx_mock.add_response(
        ...
        with pytest.raises(DependencyError, match="HTTP 500"):
            await inventory_client.get_topology(asset_id)

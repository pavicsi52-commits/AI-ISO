...
one validates against (Inventory, Configuration Management,
Automation, Workflow Runtime, Discovery), plus execution concurrency
and timeout bounds. Per docs/043's own SECURITY section ("Secure
credential access"), this service never stores raw credentials itself
-- every collector reads target state through another service's own
REST API using the caller's own bearer token, never a stored secret.
"""
...
class ValidationServiceSettings(BaseSettings):
    """Fields specific to this service, not covered by any shared_core.config section."""

    model_config = SettingsConfigDict(
        env_prefix="AIIOS_VALIDATION_SERVICE_", env_file=".env", extra="ignore"
    )

    host: str = Field(default="0.0.0.0")
    port: int = Field(default=8014, ge=1, le=65_535)
    ...
    max_parallel_checks: int = Field(default=10, ge=1)
...
class Settings:
    ...
    email: EmailSettings
    service: ValidationServiceSettings


def build_settings(*, service: ValidationServiceSettings | None = None) -> Settings:
    ...
    return Settings(
        ...
        email=shared.email,
        service=service if service is not None else ValidationServiceSettings(),
    )
...
__all__ = ["Settings", "ValidationServiceSettings", "build_settings", "get_settings"]

...
class OrganizationServiceSettings(BaseSettings):
    ...
    model_config = SettingsConfigDict(
        env_prefix="AIIOS_ORG_SERVICE_", env_file=".env", extra="ignore"
    )

    host: str = Field(default="0.0.0.0")
    port: int = Field(default=8004, ge=1, le=65_535)
...
class Settings:
    ...
    email: EmailSettings
    service: OrganizationServiceSettings


def build_settings(*, service: OrganizationServiceSettings | None = None) -> Settings:
    ...
    return Settings(
        ...
        email=shared.email,
        service=service if service is not None else OrganizationServiceSettings(),
    )
...
__all__ = ["OrganizationServiceSettings", "Settings", "build_settings", "get_settings"]

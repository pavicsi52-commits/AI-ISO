...
class TestReadiness:
    ...
    async def test_readiness_reports_a_failed_cache_check_when_redis_is_unreachable(
        ...
        assert resp.status_code == HTTP_OK
        data = resp.json()["data"]
        # Redis backs only rate limiting and the sweeps' own leader
        # election -- losing it makes readiness report the failure
        # without gating overall status, since PostgreSQL alone actually
        # gates traffic.
        assert data["status"] == "ready"
        checks_by_name = {check["name"]: check for check in data["checks"]}

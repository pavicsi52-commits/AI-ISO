...
async def scheduler() -> AsyncIterator[SchedulerManager]:
    ...
    manager = create_scheduler_framework(
        queue.manager, cache.client, queue_name="api_gateway_service_test_queue"
    )
    yield manager
...
class TestJobRegistration:
    async def test_all_three_jobs_register_under_deterministic_ids(
        self, scheduler: SchedulerManager
    ) -> None:
        # Deterministic, so re-registering on a restart replaces the job
        # rather than leaking a second copy of it.
        register_health_probe_sweep(scheduler, _noop, interval_seconds=15)
        register_statistics_rollup(scheduler, _noop, interval_seconds=900)
        register_quota_reset_sweep(scheduler, _noop, interval_seconds=3_600)
        ...
        assert scheduler.registry.get(QUOTA_RESET_SWEEP_JOB_ID).job_type is JobType.SYSTEM
        assert len(scheduler.registry.list_jobs()) == 3
    ...
    async def test_registration_computes_a_first_due_time(
        ...
        job = register_quota_reset_sweep(scheduler, _noop, interval_seconds=3_600)
    ...
    async def test_each_jobs_metadata_names_its_own_component(
        ...
        assert scheduler.registry.get(HEALTH_PROBE_SWEEP_JOB_ID).metadata["component"] == (
            "gateway-health-probe-sweep"
        )
        assert scheduler.registry.get(STATISTICS_ROLLUP_JOB_ID).metadata["component"] == (
            "gateway-statistics-rollup"
        )
        assert scheduler.registry.get(QUOTA_RESET_SWEEP_JOB_ID).metadata["component"] == (
            "gateway-quota-reset-sweep"
        )
    ...
    async def test_a_non_positive_interval_is_refused(
        ...
        with pytest.raises(ValueError, match="must be positive"):
            register_quota_reset_sweep(scheduler, _noop, interval_seconds=interval)

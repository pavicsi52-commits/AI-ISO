...
**PostgreSQL gates readiness.** Every routing decision this service
makes reads its own route/service configuration; a replica taking
traffic without a database would silently refuse every request rather
than fail loudly at the readiness probe.

**Redis is reported but does not gate.** It backs rate limiting and the
health-probe/statistics/quota-reset sweeps' leader election; losing it
makes rate limiting fail open (see :mod:`app.services.ratelimit`) and
sweeps late, not the gateway itself wrong.
"""
...
async def health(
    ...
    """Return the overall health of the API gateway service."""
    data = HealthStatus(
        status="healthy",
